function heartDelivery(array) {
    ...
    while (command !== 'Love!') {
        ...
        jumps += Number(action[1]);

        if (jumps > neighborhood.length - 1) {
            jumps = 0;
        }
...
//   Every house in the neighborhood needs a certain number of hearts delivered by
...
// •	If the needed hearts for a certain house become equal to 0, print on
// the console "Place {house_index} has Valentine's day."
// •	If Cupid jumps to a house where the needed hearts are already 0, print on
// the console "Place {house_index} already had Valentine's day."
