function meetings(array) {
    
    let meetInfo = {};
    for (let element of array) {
        
        let [day,name] = element.split(' ');

        if (!meetInfo.hasOwnProperty(day)) {
            meetInfo[day] = name;
            console.log(`Scheduled for ${day}`);
        }else{
            console.log(`Conflict on ${day}!`);
        }

        
    }
    for (let [day,name] of Object.entries(meetInfo)) {
        console.log(`${day} -> ${name}`);
    }
}

meetings(['Monday Peter',
'Wednesday Bill',
'Monday Tim',
'Friday Tim']
)
meetings(['Friday Bob',
'Saturday Ted',
'Monday Bill',
'Monday John',
'Wednesday George']
)

// Write a function that manages meeting appointments.
//  The input comes as an array of strings.
//   Each string contains a weekday and person’s name.
//    For each successful meeting, print a message. 
//    If you receive the same weekday twice, the meeting 
//    cannot be scheduled so print a conflicting message.
//     In the end, print a list of all successful meetings. 